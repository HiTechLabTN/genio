...
import { nextPose, SPRING_BY_CONTEXT, type PoseTarget } from "../../lib/mascotAnimator";
 ...
 */
function PhysicsDriver({ onPosition, target, gainRef }: { onPosition: (x: number, y: number) => void; target: { current: { x: number; y: number; z: number } }; gainRef: { current: number } }) {
    ...
    const tick = (t: number) => {
      ...
      if (body) {
        ...
        if (t - lastImpulseAt.current > 400) {
          ...
          // Gain variable selon le contexte (posé en thinking, vif en error) :
          // la dérive physique reste cohérente avec l'énergie du clip squelette
          // (pas de glissade rapide pendant une pose lente).
          const g = gainRef.current;
          body.applyImpulse({ x: dx * 0.02 * g, y: dy * 0.02 * g, z: dz * 0.015 * g }, true);
        }
...
export default function MascotStage({ chat, agentStatus, sendPrompt, onSwitchToTechnicalMode }: MascotStageProps) {
  ...
  // Ressort + agilité physique suivent le contexte (posé en thinking, vif en error)
  const spring = SPRING_BY_CONTEXT[pose?.contextKey ?? "idle"] ?? SPRING_BY_CONTEXT.idle;
  useEffect(() => {
    gainRef.current = spring.stiffness / 60;
  }, [spring.stiffness]);
    ...
    <ErrorBoundary name="RiggedMascot-stage">
      ...
      >
        <div style={{ width: 520, height: 520, position: "relative" }}>
          <HoloPlatform width={420} top={330} />
  ...
  return (
    <div className="relative h-screen w-screen overflow-hidden bg-[#020B1E]">
      ...
      {/* NOTE: le centrage utilise calc(50% - 260px) plutôt que -translate-x/y-1/2 —
          framer-motion écrit directement `transform` via style pour animer x/y, ce qui
          écraserait un translate(-50%,-50%) posé par une classe Tailwind (même règle CSS,
          le style inline gagne). Le décalage physique (pos.x/pos.y) reste géré par animate. */}
      <motion.div
        className="absolute"
        style={{ left: "calc(50% - 260px)", top: "calc(38% - 260px)" }}
