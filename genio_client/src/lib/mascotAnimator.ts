...
/** Clip fade duration per context, ms — fast for `error`, slow/posée for `thinking`. */
export const TRANSITION_MS: Record<MascotContext, number> = {
  idle: 600,
  greeting: 400,
  listening: 500,
  thinking: 750,
...
/** Motion spring tuning per context (matches the framer-motion wrapper in MascotStage). */
export const SPRING_BY_CONTEXT: Record<MascotContext, { stiffness: number; damping: number }> = {
  idle: { stiffness: 55, damping: 19 },
  greeting: { stiffness: 70, damping: 17 },
  listening: { stiffness: 55, damping: 19 },
  thinking: { stiffness: 38, damping: 22 },
  executing: { stiffness: 60, damping: 18 },
  success: { stiffness: 70, damping: 16 },
  error: { stiffness: 120, damping: 14 },
  speaking: { stiffness: 65, damping: 17 },
};
...
export interface PoseTarget {
  ...
  durationMs: number;
  /** Clip cross-fade for this transition, ms (from TRANSITION_MS). */
  fadeMs: number;
